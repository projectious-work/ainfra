...
func helpFor(topic string) (output.Help, bool) {
	...
	topics := map[string]output.Help{
		...
		"mcp.serve": command("mcp.serve", "ainfra mcp serve --stdio [--project PATH] [--capability GROUP] [--authorization-trust PATH]",
			...
			output.HelpOption{Names: []string{"--authorization-trust"}, ValueName: "PATH", Summary: "Trust Ed25519 MCP approval issuers."}),
