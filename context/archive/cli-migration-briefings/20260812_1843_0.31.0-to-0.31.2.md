...
**Status:** pending
**aibox CLI:** v0.31.0 → v0.31.2
**processkit:** v0.28.5

## Summary

aibox has been updated from v0.31.0 to v0.31.2. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.31.0 and v0.31.2 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.31.2 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.31.2` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.31.2"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.31.2.
