...
**Status:** pending
**aibox CLI:** v0.32.6 → v0.33.2
**processkit:** v0.28.6

## Summary

aibox has been updated from v0.32.6 to v0.33.2. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.32.6 and v0.33.2 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.33.2 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.33.2` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.33.2"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.33.2.
