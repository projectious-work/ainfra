...
**Status:** pending
**aibox CLI:** v0.28.13 → v0.31.0
**processkit:** v0.28.4

## Summary

aibox has been updated from v0.28.13 to v0.31.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.28.13 and v0.31.0 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.31.0 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.31.0` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.31.0"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.31.0.
