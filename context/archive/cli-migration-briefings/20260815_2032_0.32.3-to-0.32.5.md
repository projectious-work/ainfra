...
**Status:** pending
**aibox CLI:** v0.32.3 → v0.32.5
**processkit:** v0.28.6

## Summary

aibox has been updated from v0.32.3 to v0.32.5. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.32.3 and v0.32.5 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.32.5 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.32.5` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.32.5"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.32.5.
