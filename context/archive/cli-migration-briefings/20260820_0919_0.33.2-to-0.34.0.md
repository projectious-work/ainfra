...
**Status:** pending
**aibox CLI:** v0.33.2 → v0.34.0
**processkit:** v0.28.8

## Summary

aibox has been updated from v0.33.2 to v0.34.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.33.2 and v0.34.0 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Verify `/workspace/context/templates/processkit/v0.28.8/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.28.8/AGENTS.md context/templates/processkit/v0.28.8/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.28.8` (upgraded from `v0.28.8` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.28.8`)
