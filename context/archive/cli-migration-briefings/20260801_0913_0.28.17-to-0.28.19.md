...
**Status:** pending
**aibox CLI:** v0.28.17 → v0.28.19
**processkit:** v0.28.4

## Summary

aibox has been updated from v0.28.17 to v0.28.19. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.28.17 and v0.28.19 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.28.19 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.28.19` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.28.19"`
...
processkit is pinned to `v0.28.4` (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.28.19.
